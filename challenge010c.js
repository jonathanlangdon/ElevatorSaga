...
// 10c -> elevator1 -> don't stop on passing on the way down, just go to 0
...
({
  init: function (elevators, floors) {
    ...
    elevator0.on('passing_floor', (floorNum, direction) =>
      stopIfHasCapacity(elevators, floors, floorNum, elevator0, direction)
    );

    elevator0.on('idle', () =>
      goToClosestRequest(elevators, elevator0, floors)
    );

    elevator1.on('idle', () => elevator1.goToFloor(0));
...
function lowCapacityGoBackToZero(elev) {
  if (elev.loadFactor() < 0.5) elev.goToFloor(0);
}
...
function stopIfHasCapacity(elevators, floors, floorNum, elev, direction) {
  ...
  const hasCapacity = elev.loadFactor() < elev.maxPassengerCount() * 0.13;
